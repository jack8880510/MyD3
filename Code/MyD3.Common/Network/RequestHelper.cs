...
namespace MyD3.Common.Network
{
    ...
    {
        /// <summary>
        /// 将请求中标记了RequestPropertyAttribute的属性序列化成Url参数
        /// </summary>
        /// <param name="request">请求对象</param>
        /// <returns>参数字符串</returns>
        public static string ToUrlParams(BaseRequest request)
        {
            string result = "";

            var properties = request.GetType().GetProperties().Where(x => x.GetCustomAttributes(typeof(RequestPropertyAttribute), true).Length > 0).ToArray();
            ...
            foreach (var p in properties)
            {
                ...
                var objValue = p.GetValue(request);

                if (pa.ExcludeIfNull && objValue == null)
                {
                    //如果配置为NULL排除，并且值为NULL则跳过
                    continue;
                }
                ...
                {
                    //如果标记为Json序列化
                    value = JsonConvert.SerializeObject(objValue);
                }
                else
                {
        ...
        {
            //Logger.LogDebug("正在转换请求，类型为：" + typeof(T).Name, "RequestHelper", "ParseRequest");
            ...
            //Logger.LogDebug("正在创建类型的空实例", "RequestHelper", "ParseRequest");
            //创建实例
            ...
            foreach (var p in properties)
            {
                ...
                //Logger.LogDebug("正在处理属性 " + key + " 值为 " + HttpUtility.UrlDecode(source[key]), "RequestHelper", "ParseRequest");

                if (pa.NoSet)
                {
                    //Logger.LogDebug("属性 " + key + " 被设置为NOSET，跳过值设置", "RequestHelper", "ParseRequest");
                    continue;
                }
                ...
                else if (pa.ExcludeIfNull && source[key] == null)
                {
